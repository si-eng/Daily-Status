resource "aws_ecs_task_definition" "Ecstask2" {
  family                   = "Ecstask2"
  network_mode             = "awsvpc"
...
# ECS SERVICE 2
resource "aws_ecs_service" "ecs2" {
  name            = var.service2
  cluster         = "${aws_ecs_cluster.ecs-cluster.id}"
  task_definition = "${aws_ecs_task_definition.ecs2.id}"
  desired_count   = 2
  launch_type     = "FARGATE"
...
resource "aws_codebuild_project" "prod2" {
  badge_enabled  = false
  build_timeout  = 60
  name           = "prod2"
  queued_timeout = 480
