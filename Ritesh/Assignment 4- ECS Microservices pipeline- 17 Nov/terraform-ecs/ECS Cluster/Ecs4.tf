resource "aws_ecs_task_definition" "ECS4" {
  family                   = "ECS4"
  network_mode             = "awsvpc"
 ...
 container_definitions = jsonencode([
    {
      ...
      image  = "033746436249.dkr.ecr.us-east-2.amazonaws.com/dockerfile4:latest" 
      cpu    = 256
      memory = 512
      portMappings = [
        {
...
# ECS SERVICE 4
resource "aws_ecs_service" "ecs4" {
  name            = var.service4
  cluster         = "${aws_ecs_cluster.ecs-cluster.id}"
  task_definition = "${aws_ecs_task_definition.ecs4.id}"
  desired_count   = 2
  launch_type     = "FARGATE"
...
resource "aws_codebuild_project" "prod4" {
  badge_enabled  = false
  build_timeout  = 60
  name           = "prod4"
  queued_timeout = 480
