...
resource "aws_ecs_service" "service1" {
  ...
  load_balancer {
    target_group_arn = "${aws_lb_target_group.tggroup0.arn}"
...
resource "aws_codebuild_project" "docker_build1" {
  ...
  source {
    type            = "GITHUB"
    location        = "https://github.com/ShubhamChouhan9001/Docker-6-services.git"
    git_clone_depth = 1
  }
}
...
resource "aws_cloudwatch_metric_alarm" "cpu_low6" {
  alarm_name          = "cpu-low3"
  comparison_operator = "LessThanOrEqualToThreshold"
  evaluation_periods  = 3
  metric_name         = "CPUUtilization3"
...
resource "aws_cloudwatch_metric_alarm" "cpu_high6" {
  alarm_name          = "cpu-high3"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 3                              
  metric_name         = "CPUUtilization3"
