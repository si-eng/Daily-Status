resource "aws_ecrpublic_repository" "repo-6" {
...
resource "aws_ecs_task_definition" "task5" {
  ...
  container_definitions = jsonencode([
    {
      name   = "service6"
      image  = "${aws_ecrpublic_repository.repo-6.repository_uri}" 
      cpu    = 256
      memory = 512
      portMappings = [
        {
...
resource "aws_codebuild_project" "my_docker5_build5" {
  badge_enabled  = false
  build_timeout  = 60
  name           = "my_docker5_build5"
  queued_timeout = 480
