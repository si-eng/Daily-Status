#                  repository
resource "aws_ecrpublic_repository""ecr-repo-3" {
  repository_name = "ecr-repo-3"
}



resource "aws_ecs_task_definition" "task-3" {
  family                   = "HTTPserver-3"
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([
    {
      name   = "container-3"
      image  = aws_ecrpublic_repository.ecr-repo-3.repository_uri#URI #####################################################3
      cpu    = 256
      memory = 512
      portMappings = [
        {
...
resource "aws_ecs_service" "svc-3" {
  name            = "Service-3"
  cluster         = "${aws_ecs_cluster.ecs-cluster-main.id}"
  task_definition = "${aws_ecs_task_definition.task-3.id}"
  desired_count   = 1
  launch_type     = "FARGATE"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ...
        ServiceName = "svc-3"
        FileName    = "imagedefinitions-3.json"
      }
    }
  }
}
