#                  repository
resource "aws_ecrpublic_repository""ecr-repo-5" {
  repository_name = "ecr-repo-5"
}



resource "aws_ecs_task_definition" "task-5" {
  family                   = "HTTPserver-5"
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([
    {
      name   = "container-5"
      image  = aws_ecrpublic_repository.ecr-repo-5.repository_uri#URI #####################################################3
      cpu    = 256
      memory = 512
      portMappings = [
        {
...
resource "aws_ecs_service" "svc-5" {
  name            = "Service-5"
  cluster         = "${aws_ecs_cluster.ecs-cluster-main.id}"
  task_definition = "${aws_ecs_task_definition.task-5.id}"
  desired_count   = 1
  launch_type     = "FARGATE"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ...
        ServiceName = "svc-5"
        FileName    = "imagedefinitions-5.json"
      }
    }
  }
}
